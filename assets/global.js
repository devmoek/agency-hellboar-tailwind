...
entryForm.addEventListener("submit", (e) => {
  e.preventDefault();
  // Here you would normally submit to your Shopify backend
...
newsletterForm.addEventListener("submit", (e) => {
  e.preventDefault();
  modal.style.display = "none";
  alert("Thanks for subscribing! Your 5 BONUS entries have been added!");
});
...
// Exit intent popup (show modal when cursor moves to top of page)
let showExitPopup = true;

document.addEventListener("mouseleave", (e) => {
  if (e.clientY < 50 && showExitPopup) {
    modal.style.display = "flex";
    showExitPopup = false; // Only show once
  }
});
...
// Track engagement time and show reminders
let timeOnPage = 0;

setInterval(() => {
  timeOnPage += 1;

  // Show a reminder every 2 minutes
  if (timeOnPage % 120 === 0) {
    ...
    reminder.style = `
                      ...
                      bottom: 100px;
                      right: 30px;
                      background-color: rgba(229, 23, 44, 0.9);
                      color: white;
                      padding: 15px;
                      border-radius: 5px;
                      z-index: 100;
                  ...
                  `;
    reminder.innerHTML = `
                      <p><strong>Don't miss out!</strong> Time is running out to enter our car giveaway!</p>
                      <button style="
...
function createUpsellBanner() {
  ...
  upsell.innerHTML = `
                  <div>
                      <strong style="color: #e5172c;">FLASH SALE:</strong> 
                      <span>Buy our Premium Collection and get <strong>10X ENTRIES</strong> in the car giveaway!</span>
                  </div>
                  <button style="
  ...
  upsell.querySelector("button").addEventListener("click", () => {
    // Redirect to premium collection
    alert("Redirecting to Premium Collection...");
    // window.location.href = '/collections/premium';
  });
}

// Show upsell after 30 seconds
setTimeout(createUpsellBanner, 30000);

// Initialize urgency timer
function startUrgencyTimer() {
  const minutes = Math.floor(Math.random() * 10) + 5; // 5-15 minutes
  ...
  function updateUrgencyTimer() {
    ...
    urgencyTimer.innerHTML = `
            <p>🔥 <strong>${min}:${
      sec < 10 ? "0" + sec : sec
    }</strong> until next price change</p>`;

    totalSeconds--;

    if (totalSeconds < 0) {
      document.body.removeChild(urgencyTimer);
      setTimeout(startUrgencyTimer, 5000); // Restart after 30 seconds
    }
  }
  ...
  document.body.appendChild(urgencyTimer);
}

// Start urgency timer after 20 seconds
setTimeout(startUrgencyTimer, 5000);
